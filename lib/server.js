...
const createRequestHandler = flags => {
	...
	return (request, res) => {
		...
		if (isMarkservUrl) {
			...
			send(request, markservRelFilePath).pipe(res);
			return;
		}
		...
		if (isMarkdown) {
			...
			send(request, filePath, {dotfiles: 'allow'}).pipe(res);
		}
	};
};
...
const startHTTPServer = (connectApp, port, flags) => {
	...
	httpServer = connectApp ? http.createServer(connectApp) : http.createServer();
...
const doUpgrade = (newerVersion, flags) => {
	...
	message(chalk.bgRed('✨UPGRADE✨'), 'Upgrade beginning...', flags);
	const ls = spawn('npm', ['i', '-g', `markserv@${newerVersion}`], {stdio: [0, 1, 2]});

	ls.on('exit', code => {
		if (code) {
			return message(chalk.bgRed('✨UPGRADE✨'), 'Markserv could not upgrade.', flags);
		}

		message(chalk.bgRed('✨UPGRADE✨'), 'Upgrade finished!', flags);
	});
};
...
const optionalUpgrade = async flags => {
	...
	message('upgrade', 'checking for upgrade...', flags);

	return checkForUpgrade(flags).then(async version => {
		if (version === false) {
			message('upgrade', 'no upgrade available', flags);
			return;
		}

		message(chalk.bgRed('✨UPGRADE✨'), `Markserv version: ${version} is available!`, flags);

		const logInstallNotes = () => {
			message(chalk.bgRed('✨UPGRADE✨'), 'Upgrade cancelled. To upgrade manually:', flags);
			message(chalk.bgRed('✨UPGRADE✨'), chalk.bgYellow.black.bold(` npm i -g markserv@${version} `), flags);
			message(chalk.bgRed('✨UPGRADE✨'), chalk.bgYellow.black.bold(` yarn global add markserv@${version} `), flags);
		};
