...
const createPathTags = path => {
	let tags = '/'
	let url = '/'
	const names = path.replace(/(^\/+|\/+$)/g, '').split('/')
	if (names[0]) {
...
const createRequestHandler = flags => {
	...
	return (req, res) => {
		...
		const errorPage = (code, err) => {
			...
			const url = unescape(req.headers.referer || path.parse(decodedUrl).dir)
			const msg = md.utils.escapeHtml(String(err))
		...
		const decodedReqUrl = unescape(req.url)
		const isMarkservUrl = decodedReqUrl.includes(markservUrlLead)
		if (isMarkservUrl) {
			const markservFilePath = decodedReqUrl.split(markservUrlLead)[1]
			const markservRelFilePath = path.join(__dirname, markservFilePath)
			if (flags.verbose) {
				msg('{markserv url}', style.link(markservRelFilePath), flags)
			}
			send(req, markservRelFilePath, {
				immutable: true,
				maxAge: '1d'
			}).pipe(res)
			return
...
const init = async flags => {
	const liveReloadPort = Number(flags.livereloadport)
