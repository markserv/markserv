...
const icons = require(path.join(__dirname, 'icons', 'material-icons.json'))

// const getIconByDataUri = {}

const iconCSS = []
Reflect.ownKeys(icons.iconDefinitions).forEach(iconName => {
	const iconFile = icons.iconDefinitions[iconName].iconPath
console.log(`.icon.${iconName}:before {
    content: "";
    display: block;
    position: absolute;
    background: url("${iconFile}") no-repeat;
    width: 20px;
    height: 20px;
    float: left;
    margin: 0 6px 0 0;
...
const isMarkservUrl = url => indexOf('markserv://' === 0)
const getFilePath = markservUrl => {
	return markservUrl.split('markserv://')[0]
}
...
const createRequestHandler = flags => {
	...
	const implantHandlers = {
		markserv: (prop, opts) => new Promise(resolve => {
			// const relUrl = path.relative(__dirname, opts.relPath)
	...
	return (req, res) => {
		...
		console.log(isMarkservUrl, req.url, req.originalUrl, filePath)

		if (isMarkservUrl) {
			const markservFilePath = req.url.split(markservUrlLead)[1]
			const markservRelFilePath = path.join(__dirname, markservFilePath)

			console.log(markservFilePath)
			console.log(markservRelFilePath)
		...
		if (isMarkdown) {
			...
			send(req, markservRelFilePath || filePath).pipe(res)
		}
	}
}
